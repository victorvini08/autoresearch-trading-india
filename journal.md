...
## Iteration 2026-05-16-ba8bb6b — REVERTED

**Hypothesis:** Making the macro regime gate symmetric by exiting existing holdings during risk_off or shock regimes should improve validation Sortino by directly reducing the broad-market drawdown tail that prior rank-only momentum variants failed to control.

**Change:** I removed the disallowed pathlib import and changed the regime logic so risk_off or shock classifications liquidate held positions at rebalance instead of merely blocking fresh entries, while preserving PIT universe selection, biweekly cadence, sector cap, equal sizing, and order_target_percent-only execution.
...
## Iteration 2026-05-16-0b92c99 — REVERTED

**Hypothesis:** Ranking candidates by 12-1 momentum divided by realized volatility should improve validation Sortino by preserving the momentum thesis while reducing the high-volatility winner concentration that has repeatedly driven drawdown failures.

**Change:** I replaced raw momentum ranking with a no-new-parameter risk-adjusted momentum score and removed the disallowed pathlib dependency while preserving PIT universe selection, biweekly cadence, sector cap, retention, equal sizing, and order_target_percent-only execution.

**Decision:** REVERTED — catastrophe: max drawdown: 63.3% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.1724 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.28)

**Result:**
- validation_sortino_mean: 2.0869265391364062
- validation_folds: 20
- per_fold_sortinos: [9.6457, 2.6603, 0.0264, 1.5281, -0.4638, -1.4669, -1.9454, -0.1135, -1.5044, -2.0607, 3.5624, 13.4013, 3.1316, 3.1229, 2.9886, 2.6216, 1.4006, 2.0303, 2.1031, 1.0702]
- calmar_mean: 4.873375484389376
- hit_rate_mean: 0.16833333333333333
- profit_factor_mean: 3.980040975397615
- trade_count_total: 27
- aggregate_max_dd: 0.6329346642370864
- worst_fold_max_dd: 0.3607359288792167
- max_position_frac_peak: 1.0404314311657057
- lower_quartile_fold_calmar: -1.3474019254324743
- n_negative_folds: 7/20
- risk.passed: False
- risk.violations: ['max drawdown: 63.3% > 50% (account-wipe territory)']

**Learning:** Sortino scored 2.087 with no prior kept baseline. Aggregate DD was 63.3%; negative folds were 7/20; trades=27. Do not reuse this exact setup: it failed the catastrophe gate, so the result is not a usable edge even if the hypothesis was economically plausible. Decision reason: catastrophe: max drawdown: 63.3% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.1724 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.28).

---

## Iteration 2026-05-16-2f3bc19 — REVERTED

**Hypothesis:** Scaling portfolio gross exposure down as point-in-time India VIX percentile rises should improve validation Sortino by attacking the repeated drawdown failure at the portfolio-construction layer without changing the already-tested momentum signal family.

**Change:** I removed the disallowed pathlib dependency and replaced the fixed 99% gross target with a VIX-aware gross budget, capped further in risk_off/shock regimes, while preserving PIT universe ranking, biweekly cadence, sector cap, retention, and order_target_percent-only execution.

**Decision:** REVERTED — catastrophe: max drawdown: 70.2% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.4075 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.11)

**Result:**
- validation_sortino_mean: 1.7908354901671644
- validation_folds: 20
- per_fold_sortinos: [13.937, 5.8886, 0.0989, -0.4852, -4.5207, -3.6679, -2.343, -2.4588, -2.3367, -1.4097, 7.2267, 16.3485, 5.0925, 3.8981, 5.3312, 3.1544, -0.3536, -0.1365, -3.1134, -4.3337]
- calmar_mean: 6.799618039630339
- hit_rate_mean: 0.37583333333333335
- profit_factor_mean: 1.1400481667265798
- trade_count_total: 72
- aggregate_max_dd: 0.7016350253514761
- worst_fold_max_dd: 0.2973972195481254
- max_position_frac_peak: 0.8636477775174118
- lower_quartile_fold_calmar: -2.0501394267172612
- n_negative_folds: 11/20
- risk.passed: False
- risk.violations: ['max drawdown: 70.2% > 50% (account-wipe territory)']

**Learning:** Sortino scored 1.791 with no prior kept baseline. Aggregate DD was 70.2%; negative folds were 11/20; trades=72. Do not reuse this exact setup: it failed the catastrophe gate, so the result is not a usable edge even if the hypothesis was economically plausible. Decision reason: catastrophe: max drawdown: 70.2% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.4075 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.11).

---

## Iteration 2026-05-16-2991fdf — REVERTED

**Hypothesis:** Using a point-in-time Nifty 50 200-DMA trend gate to go fully defensive when the broad market is below trend should improve validation Sortino by reducing the recurring long-only crash drawdown without adding a tuned hyperparameter.

**Change:** I removed the disallowed pathlib dependency and added a numeric Nifty-vs-200DMA defensive gate that exits all holdings and blocks new entries when the index is below its 200-day average, while preserving biweekly ranking, sector caps, PIT universe handling, and order_target_percent-only execution.
...
## Iteration 2026-05-16-5573252 — REVERTED

**Hypothesis:** Adding a no-new-parameter daily holding-level trend exit should improve validation Sortino by cutting deteriorating positions between biweekly rebalances instead of waiting up to two weeks for the cross-sectional rank to refresh.

**Change:** I removed the disallowed pathlib import and added a daily 126-day moving-average breakdown exit for held names, while keeping entries biweekly, PIT universe ranking, sector cap, and order_target_percent-only execution.

**Decision:** REVERTED — catastrophe: max drawdown: 74.3% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.2603 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.06)

**Result:**
- validation_sortino_mean: 0.6329385468661506
- validation_folds: 20
- per_fold_sortinos: [10.2078, 3.3704, 0.0715, -0.0566, -4.4618, -3.58, -1.5633, -1.0164, -1.3616, -3.7422, 1.3999, 12.1353, 3.1295, 2.7832, 2.8153, -0.9621, 0.2318, -0.6236, -3.4265, -2.6918]
- calmar_mean: 3.384357309397019
- hit_rate_mean: 0.21166666666666661
- profit_factor_mean: 0.8073221926626111
- trade_count_total: 51
- aggregate_max_dd: 0.7428963910152055
- worst_fold_max_dd: 0.34691527023482793
- max_position_frac_peak: 1.142093104431454
- lower_quartile_fold_calmar: -1.9153912594455882
- n_negative_folds: 13/20
- risk.passed: False
- risk.violations: ['max drawdown: 74.3% > 50% (account-wipe territory)']

**Learning:** Sortino scored 0.633 with no prior kept baseline. Aggregate DD was 74.3%; negative folds were 13/20; trades=51. Do not reuse this exact setup: it failed the catastrophe gate, so the result is not a usable edge even if the hypothesis was economically plausible. Decision reason: catastrophe: max drawdown: 74.3% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.2603 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.06).

---

## Iteration 2026-05-16-c76c757 — REVERTED

**Hypothesis:** Skipping fresh momentum entries with explicitly adverse point-in-time LLM news or event classifications should improve validation Sortino by avoiding idiosyncratic blow-up candidates without changing the core 12-1 momentum thesis or adding a tunable parameter.

**Change:** I removed the disallowed pathlib dependency and added a no-new-param adverse-news veto for new entries, while preserving biweekly PIT universe ranking, retention, sector cap, regime gating, and order_target_percent-only execution.

**Decision:** REVERTED — catastrophe: max drawdown: 77.7% > 50% (account-wipe territory) | anti-overfit FAILED: bonferroni(p=0.9998 >= alpha/N=0.0050) · random_walk_mc(only 0.86% percentile vs RW null) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.15)
...
## Iteration 2026-05-16-5c95393 — REVERTED

**Hypothesis:** Replacing equal-weight momentum slots with inverse-downside-volatility sizing across the selected names should improve mean validation Sortino by reducing capital assigned to historically crash-prone winners without changing the existing ranking thesis or adding a tuned hyperparameter.

**Change:** I removed the disallowed pathlib import and changed portfolio construction so selected names keep the same biweekly momentum/quality/sector selection, but receive normalized inverse downside-volatility weights capped per name and sector before order_target_percent orders are issued.

**Decision:** REVERTED — anti-overfit FAILED: bonferroni(p=0.9984 >= alpha/N=0.0050) · sub_period_stationarity(min/max ratio of |Sortino| across 3 sub-periods = 0.13)

**Result:**
- validation_sortino_mean: 1.2820485178794319
- validation_folds: 20
- per_fold_sortinos: [9.1974, 3.7602, 0.0777, 0.4993, -0.8863, -2.0346, -2.949, -1.8025, -1.9847, -2.8005, 6.5251, 11.8681, 4.23, 2.3283, 3.897, 1.9454, 0.7053, -0.1885, -2.8425, -3.9042]
- calmar_mean: 1.40180784864508
- hit_rate_mean: 0.37470238095238095
- profit_factor_mean: 2.0415902971179003
- trade_count_total: 73
- aggregate_max_dd: 0.2759257518691613
- worst_fold_max_dd: 0.10904822996551945
- max_position_frac_peak: 0.2581938648859716
- lower_quartile_fold_calmar: -1.9281449917035456
- n_negative_folds: 9/20
- risk.passed: True
- risk.violations: []
