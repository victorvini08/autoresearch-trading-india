...
## Iteration 2026-05-19-c5df59b — REVERTED

**Hypothesis:** Adding a second, orthogonal parameter-free slope-confirmed breakdown gate computed on the equal-weight cumulative-return index of the QUALIFIED momentum-quality long leg (not the broad universe) — taking gross fully to cash on a confirmed slope-down momentum-factor unwind — will raise the worst disjoint sub-period Sortino without regressing aggregate drawdown or the strong folds, because the canonical Daniel-Moskowitz (2016) momentum crash occurs while the broad market is flat/rebounding (so the existing broad-universe market gate is structurally blind to it) yet the momentum long leg specifically reverses, which is exactly the residual bear/rebound channel that sets the negative folds.

**Change:** In next() only, after the existing broad-market regime gate, I added a momentum-FACTOR breakdown gate that reuses the exact proven parameter-free slope-confirmed `_market_regime_breakdown` logic but feeds it the qualified momentum long-leg (`priority`) closes instead of the broad universe, setting gross to 0.0 on a confirmed momentum-factor breakdown the market gate cannot see — no new import, no new tunable hyperparameter, strictly one-sided (can only lower gross), and byte-inert whenever the momentum long leg is in a healthy rising-MA trend.
...
## Iteration 2026-05-19-d93afad — REVERTED

**Hypothesis:** Sector-demeaning only the two pure momentum inputs (long_mom, mid_mom) before their cross-sectional percentile rank — while leaving the raw absolute-momentum entry qualification and every quality component untouched — will raise the worst disjoint sub-period Sortino without regressing aggregate drawdown or turnover, because cross-sectional 12-1 selection carries an implicit time-varying sector bet that reverses together and sets the negative folds, and the 25% sector cap bounds only final exposure, not which names the rank prefers.

**Change:** In momentum_quality_scores I added an optional PIT sector map and rank long_mom/mid_mom on their sector-demeaned values (best name WITHIN each sector) instead of raw across-sector momentum, a parameter-free thesis change distinct from the exhausted gross-overlay / added-rank-term / per-name-sizing families that targets the sector-concentrated reversal driving the worst sub-periods.

**Decision:** REVERTED — sortino 2.134 did not improve on prev 3.4927787451086587

**Result:**
- evaluator_version: 2026-05-16-univfloor
- validation_sortino_mean: 2.1344561983771686
- validation_folds: 13
- per_fold_sortinos: [-0.0201, -0.9017, -1.3046, 3.8779, 4.4812, 1.3123, 3.0148, 8.1327, 3.2889, 1.7873, 4.9248, 0.6002, -1.4459]
- calmar_mean: 3.959717236565929
- hit_rate_mean: 0.44705294705294707
- profit_factor_mean: 3.7883801040506695
- trade_count_total: 78
- aggregate_max_dd: 0.15888349142849437
- worst_fold_max_dd: 0.10454829261948807
- max_position_frac_peak: 0.10433931159175122
- lower_quartile_fold_calmar: -0.11302648396394323
- n_negative_folds: 4/13
- risk.passed: True
- risk.violations: []
...
## Iteration 2026-05-19-a3379db — REVERTED

**Hypothesis:** Generalising the KEPT slope-confirmed market-regime cash gate with a parameter-free Daniel-Moskowitz bear-state branch — cash when the broad equal-weight index is below a NON-rising (flat or falling, vs strictly-falling) structural MA AND the trailing ~1yr equal-weight market return is negative — will raise the worst disjoint sub-period Sortino without regressing drawdown or the strong folds, because the residual loss in the 2 negative early/bear folds is the first leg-down that occurs before the slow structural MA has rolled over (the strict-falling requirement lags by ~one skip-window), while the long-horizon-bear guard keeps the new branch byte-inert in every bull pullback/consolidation and the MA-non-rising condition preserves the kept gate's auto-release so the high-Sortino post-bear recovery folds are never clamped.

**Change:** In `_market_regime_breakdown` only, I OR-ed the unchanged kept slope-confirmed break with an earlier, strictly one-sided bear-state-guarded break (index < a non-rising structural MA AND trailing full-window equal-weight market return < 0), reusing only existing windows/horizons so no new hyperparameter is added and behaviour is byte-identical in every bull and every MA-rising recovery.
...
## Iteration 2026-05-19-a2d2020 — REVERTED

**Hypothesis:** Stabilising deployed breadth — bounding the per-name weight in `construct_gross_targets` by `gross / ceil(0.99/_MAX_NAME_WEIGHT)` so the same gross budget is always spread over ~10 names instead of collapsing into 3–5 highly-concentrated names whenever the vol-target/regime overlay lowers `gross` — raises the worst disjoint sub-period Sortino without regressing the strong folds, because the committed flat-cap fill makes the book MOST concentrated in exactly the turbulent low-gross bear sub-periods that set the worst Sortino, which is unrewarded idiosyncratic variance when momentum has no regime edge.

**Change:** In `construct_gross_targets` only, the per-name cap is now additionally bounded by `gross0 / _MIN_DEPLOY_BREADTH` (derived parameter-free from the two pre-committed risk constants — the 0.99 gross clip ceiling and the 10% per-name cap — adding `import math`), so the effective per-name weight is always ≤ the committed 10% cap (strictly weakly more diversified, never more concentrated, byte-~identical when gross≈0.99) and the book holds a stable ~10-name breadth at all gross levels instead of concentrating into a handful of crash-prone names under stress.

**Decision:** REVERTED — sortino 2.746 did not improve on prev 3.4927787451086587 | anti-overfit FAILED: sub_period_stationarity(signed min/max Sortino ratio across 2 sub-periods = 0.1631 (need ≥ 0.20); sub-periods = [+3.699, +0.603])

**Result:**
- evaluator_version: 2026-05-16-univfloor
- validation_sortino_mean: 2.7462509671945705
- validation_folds: 13
- per_fold_sortinos: [0.3771, -0.2976, -1.4127, 5.6691, 11.4773, 2.6499, 4.4566, 6.1446, 4.2239, 0.5517, 1.5142, 0.8604, -0.5131]
- calmar_mean: 5.166804634121734
- hit_rate_mean: 0.4241452991452991
- profit_factor_mean: 3.2201299763265334
- trade_count_total: 85
- aggregate_max_dd: 0.07359532349539229
- worst_fold_max_dd: 0.07359532349539236
- max_position_frac_peak: 0.0950137348908116
- lower_quartile_fold_calmar: 0.5538415762014237
- n_negative_folds: 3/13
- risk.passed: True
- risk.violations: []
