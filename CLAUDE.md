...
| Starting capital | **₹50,000** paper account (`dhan-paper`) — set 2026-06-11 to the user's confirmed real deployment size, so the forward dhan-paper validation runs at the capital that will actually be traded (reverted from the earlier ₹1,00,000; the `_INITIAL_DEPOSIT_BY_MODE` / `DhanExecutor` / `DhanMock` defaults all anchor ₹50k). This matches the `prepare.py` evaluator's **₹50k** `INITIAL_CASH` base; variants were additionally validated at ≥10× (₹5L) but the ₹50k gate verdict is the deployment-scale one (stationarity verdicts mirror-flip at ₹5L — see Strategy shape row). Position count is an *outcome* of bounded gross-targeting (~10–15 names when fully invested at the 10% per-name cap), not a fixed 5–6. | User-confirmed 2026-06-11; DP-charge / concentration trade-off |
