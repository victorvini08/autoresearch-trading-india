...
## Iteration 2026-05-16-6113dd6 — REVERTED

**Hypothesis:** Using today's price (no skip) in the 52-week-high proximity score — the canonical George & Hwang (2004) definition — produces a fresher, more reactive signal that immediately penalizes stocks that peaked 3 weeks ago and are declining, improving mean validation Sortino by reducing the 21-day lag that delays both entry into accelerating positions and exit from decelerating ones.

**Change:** Changed `_high52_proximity_for` to compute score as `close[0] / max(close[0..lookback_days])` (today's price vs 252-day high including today) instead of `close[-skip_days] / max(close[skip_days..lookback_days+skip_days])`, implementing the original G&H measure without the raw-momentum skip convention that is not theoretically motivated for 52-week proximity; skip_days param retained for API compatibility but no longer affects the signal.

**Decision:** REVERTED — sortino 1.391 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0338 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.78)

**Result:**
- validation_sortino_mean: 1.3912117382375384
- validation_folds: 20
- per_fold_sortinos: [11.1907, 4.2259, 0.4588, 0.7686, -0.7728, -1.0529, -0.1848, 0.3373, -1.5984, -2.7474, 2.508, 5.8153, 3.5091, 5.3115, 4.2537, 1.62, -0.1461, 0.7558, -2.0247, -4.4033]
- calmar_mean: 3.1535104830576364
- hit_rate_mean: 0.2998722836583433
- profit_factor_mean: 1.4170749398466365
- trade_count_total: 368
- aggregate_max_dd: 0.3078359351860642
- worst_fold_max_dd: 0.1430762843825889
- max_position_frac_peak: 0.21360943071867625
- lower_quartile_fold_calmar: -1.1868943766621016
- n_negative_folds: 8/20
- risk.passed: True
- risk.violations: []

**Learning:** Sortino changed from 2.172 to 1.391 (-0.781). Aggregate DD was 30.8% versus previous kept 22.2%; negative folds were 8/20; trades=368. Do not repeat this exact idea without a materially different mechanism; the keep gate rejected it for the stated reason. Decision reason: sortino 1.391 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0338 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.78).

---

## Iteration 2026-05-16-f5880a6 — REVERTED

**Hypothesis:** Adding a minimum proximity score floor (0.80) as an exit condition for held positions — selling stocks whose 52-week-high proximity score has fallen below this threshold regardless of retention rank — will improve mean validation Sortino and sub-period stationarity by naturally de-risking the portfolio when individual momentum deteriorates, cutting positions where the George & Hwang anchoring premium is exhausted, without adding any new tunable hyperparameters.

**Change:** Added a module-level _PROXIMITY_EXIT_FLOOR = 0.80 constant and built a ranked_scores dict from the existing rank computation, then modified the retained list to exclude held stocks whose current proximity score is below the floor; this makes the portfolio shrink naturally in bear markets as stocks retreat >20% from their 52-week highs rather than holding through continued momentum-reversal drawdown, targeting sub_period_stationarity improvement while avoiding the parsimony-budget hit of a new parameter.

**Decision:** REVERTED — sortino 2.119 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0168 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.05)

**Result:**
- validation_sortino_mean: 2.11933712632377
- validation_folds: 20
- per_fold_sortinos: [12.2226, 3.3443, 0.3582, 0.7687, -0.8172, -0.6808, 0.2514, -0.4923, 1.5587, -2.3272, 4.9112, 11.2162, 3.1341, 3.8208, 4.3061, 1.6074, 0.4337, 1.6485, -0.3075, -2.5701]
- calmar_mean: 3.1590941327065503
- hit_rate_mean: 0.4025659930296661
- profit_factor_mean: 1.8032855509494035
- trade_count_total: 347
- aggregate_max_dd: 0.23357574419411564
- worst_fold_max_dd: 0.14478441967790515
- max_position_frac_peak: 0.20443320438446383
- lower_quartile_fold_calmar: -0.538198040064948
- n_negative_folds: 6/20
- risk.passed: True
- risk.violations: []

**Learning:** Sortino changed from 2.172 to 2.119 (-0.053). Aggregate DD was 23.4% versus previous kept 22.2%; negative folds were 6/20; trades=347. Do not repeat this exact idea without a materially different mechanism; the keep gate rejected it for the stated reason. Decision reason: sortino 2.119 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0168 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.05).

---

## Iteration 2026-05-16-4cc8ab1 — REVERTED

**Hypothesis:** Switching to regime-adaptive rebalancing — monthly (28-day minimum) when Nifty is above its 200-DMA to reduce DP charge drag, and biweekly (14-day minimum) when below to exit deteriorating positions faster — will improve mean validation Sortino above 2.172 and fix the sub-period stationarity failure seen in prior fixed-monthly attempts, because the bear-regime acceleration prevents the long holding of declining positions that degraded sub-period Sortinos while preserving the full DP-savings benefit during bull periods.

**Change:** Replaced the fixed parity-based biweekly schedule in `_is_rebalance_today` with a date-gap check that uses the regime gate already computed per bar: 28-day minimum gap (monthly) when `allow_new=True` (Nifty above 200-DMA, bull) and 14-day minimum gap (biweekly) when `allow_new=False` (bear), with no new hyperparameters added and the `_week_parity_initialized` bookkeeping removed since it is no longer needed.

**Decision:** REVERTED — sortino 1.737 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0190 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.44)

**Result:**
- validation_sortino_mean: 1.737032353805055
- validation_folds: 20
- per_fold_sortinos: [10.9588, 4.202, 0.3584, 0.7338, -0.8124, -0.1539, 1.089, 0.6646, -0.0407, -2.8989, 5.739, 2.3023, 3.9846, 6.9299, 5.9201, 1.5931, -0.3575, 1.1153, -2.5216, -4.0652]
- calmar_mean: 3.673998431360213
- hit_rate_mean: 0.37363562972115605
- profit_factor_mean: 1.6629331560171305
- trade_count_total: 200
- aggregate_max_dd: 0.2307376033619702
- worst_fold_max_dd: 0.16590959263041757
- max_position_frac_peak: 0.21463380621934708
- lower_quartile_fold_calmar: -0.5250281064114877
- n_negative_folds: 7/20
- risk.passed: True
- risk.violations: []

**Learning:** Sortino changed from 2.172 to 1.737 (-0.435). Aggregate DD was 23.1% versus previous kept 22.2%; negative folds were 7/20; trades=200. Do not repeat this exact idea without a materially different mechanism; the keep gate rejected it for the stated reason. Decision reason: sortino 1.737 did not improve on prev 2.172090958313302 | anti-overfit FAILED: bonferroni(p=0.0190 >= alpha/N=0.0050) · parsimony(baseline params=7, strategy=7; needs Sortino +0.00, has +-0.44).
